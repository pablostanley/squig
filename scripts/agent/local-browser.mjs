...
try {
  ...
  await expect(page.getByText(/canvas.squig.json/, { exact: false }).first()).toBeHidden()
  ...
  await expect(page.getByRole("textbox", { name: "Invitation for your agent" })).toHaveValue(/canvas.squig.json/)
  await page.getByText("Connection details", { exact: true }).click()
