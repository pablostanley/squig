...
try {
  ...
  const browserDocumentId = await plain.evaluate(() => window.squig.documentId())
  const beforeInvite = await plain.evaluate(() => window.squig.serialize())
  ...
  await expect(plain.getByText("Invite your agent to this canvas. Changes save automatically in this browser.")).toBeVisible()
  await expect(plain.getByRole("button", { name: "Copy for your agent" })).toBeVisible()
  await plain.getByRole("button", { name: "Copy for your agent" }).click()
  ...
  expect(await plain.evaluate(() => window.squig.serialize())).toBe(beforeInvite)
