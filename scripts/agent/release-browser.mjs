...
try {
  ...
  await expect(page.getByRole("button", { name: "Download local file", exact: true })).toBeVisible()
  await expect(page.getByRole("button", { name: "Copy for your agent", exact: true })).toBeVisible()
  ...
  await page.waitForFunction(() => Object.keys(localStorage).some((key) => key.startsWith("squig:file:") && localStorage.getItem(key)?.includes("Release check")))
  await page.reload()
