  ...
  if (menu.nodeId) {
    entries = [
      ...
      ...(one?.type === "text" || (one?.type === "component" && getDef(one.kind)?.controls.some((c) => c.type === "text"))
        ...
        : []),
      { separator: true },
      { label: "Bring to front", hint: kbd("mod+]"), run: () => st().bringToFront(targets) },
      { label: "Bring forward", hint: kbd("alt+mod+]"), run: () => st().bringForward(targets) },
      { label: "Send backward", hint: kbd("alt+mod+["), run: () => st().sendBackward(targets) },
      { label: "Send to back", hint: kbd("mod+["), run: () => st().sendToBack(targets) },
      { separator: true },
